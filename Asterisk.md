...
**Самостоятельно установить на компьютер виртуальную машину. Мы можете использовать VMware Workstation Player или Oracle VirtualBox.**
**Операционная система Ubuntu 22.04.5 server**
...
*Рис 1.N. Настройка типа подключения*
...
Запустите на хостовом ПК программу PuTTY (необходимо предварительно её установить), в поле Host Name введите полученный на предыдущем шаге IP адрес Ubuntu, выберите тип подключения SSH и нажмите Open (Рисунок X).

![Подключение через Putty](/image/putty.png)

*Рис 1.N. Подключение через Putty*
...
В случае успешного конфигурирования в конце должна быть получена следующая картина (Рисунок X).
...
Необходимые компоненты: (**Взято из старой методички**)
...
sudo make
```
Если сборка прошла успешно, отобразится сообщение об успешном завершении и возможности установить собранный пакет (Рисунок X).
...
Если служба корректно запущена и работает, в выводе должно быть указано active (running) (Рисунок X).
```bash
...
Если регистрация клиента на сервере прошла успешно, внизу окна появится соответствующая надпись и индикатор будет зелёным, а в интерфейсе Asterisk появятся сообщения о том, что прошла регистрация и клиент с номером … стал доступен (Рисунок X).

![Выбор сетевого моста в Virtual Box](/image/phonerlite.png)

*Рис 2.8. Проверка успешной регистрации клиента на сервере*
...
Рис 3.X. Проверка возможности локального подключения к СУБД
...
*Рис 4.X. Схема взаимодействия компонентов инфраструктуры PKI*
